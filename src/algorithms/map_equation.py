...
def map_equation_old(G, partition_map):
    ...
    p_a_i, q_out_i, q_out, p_circle_i,p_u, partition_map, community_map = map_equation_essentials_old(G, partition_map)
...
def map_equation(G, partition_map):
    ...
    print(mapping)
    unique_partitions = [mapping[elem] for elem in unique_partitions]
    diagonal = np.array([mapping[elem] for elem in diagonal])
...
def retrieve_linkings(node_partition_in_links, node_partition_ex_links, node_weights, partition_mapping):
    ...
    partition_in_links = np.zeros(num_partitions)
    unique_partitions = [mapping[elem] for elem in unique_partitions]
    partition_mapping = np.array([mapping[elem] for elem in partition_mapping])
    for partition in unique_partitions:
    ...
    for node_weight, community in zip(p_u, partition_mapping):
        partition_probabilities[community] += node_weight
...
def extract_community_map_old(partition):
    v = {}
    for key, value in partition.items():
        v.setdefault(value, []).append(key)
    communities = list(dict(sorted(v.items())).values())
...
def map_equation_essentials_old(G, partition_map):
    ...
    comm2id = dict({community: idx for idx, community in enumerate(set(partition_map.values()))})
    id2comm = dict(enumerate(comm2id.keys()))
    original_community_map = dict(enumerate(extract_community_map_old(partition_map))) # For some reason partition zero misses
    community_map = {idx:[node2id[node] for node in community] for idx, community in original_community_map.items()}
    partition_map = {node2id[node]:comm2id[community] for node, community in partition_map.items()}
