...
class LouvainCoreAlgorithm:
    ...
    def run(self, G):
        ...
        if self.verbose:
            print(f"Final results are in! Algorithm reduced graph to {len(np.unique(list(result.values())))} communities")
    ...
    def local_movement(self, G, partition_map):
        ...
        while True and len(G.nodes()) > 1:
            ...
            for node in random_order:
                ...
                gains = [
                    self._compute_fitness(G, partition_map_copy, initial_fitness, node, candidate_community) for candidate_community in candidates
